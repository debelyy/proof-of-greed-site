...
const hall = readJson("hall.json", { wallets: {}, totals: { fed: 0, cashed: 0, hits: 0, fedN: 0, cashedN: 0, hitsN: 0 } });
...
for (let from = Math.max(state.lastBlock + 1, DEPLOY); from <= head; from += CHUNK) {
  ...
  for (const l of logs) {
    ...
    const a = who(l);
    const w = hall.wallets[a] || (hall.wallets[a] = { fed: 0, cashed: 0, hits: 0, big: 0, last: 0 });
    w.last = parseInt(l.blockNumber, 16);
    if (t === SIG.DEP) { w.fed += usd; hall.totals.fed += usd; hall.totals.fedN++; }
    else if (t === SIG.WD) { w.cashed += usd; hall.totals.cashed += usd; hall.totals.cashedN++; }
    else if (t === SIG.WIN) { w.hits += usd; hall.totals.hits += usd; hall.totals.hitsN++; if (usd > w.big) w.big = usd; }
  }
  state.lastBlock = to;
